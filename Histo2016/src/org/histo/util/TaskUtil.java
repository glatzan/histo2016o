...
public class TaskUtil {
	...
	result.setDateOfReceipt(System.currentTimeMillis());
	result.setDueDate(System.currentTimeMillis());
	result.setTaskID(Integer.toString(TimeUtil.getCurrentYear()) + fitString(taskNumer, 4));
    ...
    /**
     * Creates a new sample with one diagnosis an standard staingings. Adds the sample to the given tasks and returns it-
     * 
     * @param task
	...
	staining.setStainingID(block.getParent().getSampleID()+block.getBlockID()+ " " + prototype.getName() + number);
    ...
    public final static Task getLastSample(List<Task> tasks) {
	Task result = null;
	if (tasks != null && tasks.size() > 0) {
		...
		if (result == null) {
		    result = task;
		} else if (result.getDateOfReceipt() < task.getDateOfReceipt()) {
		    result = task;
		}
	    }
	}
