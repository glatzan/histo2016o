...
public class WorklistHandlerAction implements Serializable {
    ...
    /******************************************************** Patient ********************************************************/
    /**
     * Tabindex of the patient dialog
     */
    ...
    // rendered="#{worklistHandlerAction.selectedPatient ne null and worklistHandlerAction.selectedPatient.selectedTask eq null}"
    ...
    /**
     * Shows the "/pages/dialog/patient/addPatient" dialog and creates an new empty patient.
     */
    ...
    public void searchPatient(String piz){
    	...
    	String json = admim.getRequest(HistoSettings.PATIENT_GET_URL+"/"+piz);
	...
	// setzte das diasplay auf das eingangsbuch wenn der patient angezeigt wird
	...
	// TODO remove task and statningP list an use it from patient.selectedTask....
	Sample newSample = TaskUtil.createNewSample(task);

	helper.log.info("Neue Probe erstellt: TaskID: " + task.getTaskID() + ", SampleID: " + newSample.getSampleID(), log, getSelectedPatient());
	...
	helper.log.info("Neuen Block erstellt: SampleID: " + sample.getSampleID() + ", BlockID: " + block.getBlockID(), log, getSelectedPatient());
    ...
    /**
     * Archiviert einen Sample/Block/Task/Objektträger und updated die ObjektträgerListe
     * 
     * @param task
     * @param archiveAble
     * @param archived
	...
	helper.log.info("Neue Diagnose erstellt: SampleID: " + sample.getSampleID() + ", DiagnosisID/Name: " + newDiagnosis.getId() + " - " + TaskUtil.getDiagnosisName(sample, newDiagnosis), log, sample.getPatient());
    }
    ...
    /******************************************************** Contact ********************************************************/
    /**
     * Zeigt einen Dialog zum verwalten der Benachrichtigugen/Kontakte für eine Aufgabe.
     * 
     * @param task
     * @param surgeon
     * @param extern
    ...
    /**
     * Sobald im Kontaktdialog ein neuer Kontakt ausgewählt wird, wird je nach Art eine Benachrichtigung vorausgewählt.
     * 
     * @param contact
     */
	...
	if (contact.getRole() == Contact.ROLE_NONE) {
	    ...
	    if (contact.getRole() == Contact.ROLE_EXTERN && contact.getPhysician().getPerson().getFax() != null && !contact.getPhysician().getPerson().getFax().isEmpty()) {
		contact.setUseFax(true);
		return;
	    }
	    // in allen anderen fällen email setzten
	    if (contact.getPhysician().getPerson().getEmail() != null && !contact.getPhysician().getPerson().getEmail().isEmpty())
		contact.setUseEmail(true);
	}
    }
	...
	case SearchOptions.SEARCH_INDEX_STAINING:
		...
		getWorkList().addAll(patientDao.getPatientWithoutTasks(TimeUtil.setDayBeginning(cal).getTimeInMillis(), TimeUtil.setDayEnding(cal).getTimeInMillis()));
	    }

	    if (searchOptions.isStaining_staining() && searchOptions.isStaining_restaining()) {
		getWorkList().addAll(patientDao.getPatientByStainingsBetweenDates(0, System.currentTimeMillis(), false));
	    } else {
		List<Patient> paints = patientDao.getPatientByStainingsBetweenDates(0, System.currentTimeMillis(), false);
	...
	case SearchOptions.SEARCH_INDEX_TODAY:
	    getWorkList().clear();
	    getWorkList().addAll(patientDao.getWorklistDynamicallyByType(TimeUtil.setDayBeginning(cal).getTimeInMillis(), TimeUtil.setDayEnding(cal).getTimeInMillis(), searchOptions.getFilterIndex()));
	    log.info("Worklist today - " + new Date(TimeUtil.setDayBeginning(cal).getTimeInMillis()) + " " + new Date(TimeUtil.setDayEnding(cal).getTimeInMillis()));
	    break;
	case SearchOptions.SEARCH_INDEX_YESTERDAY:
	    getWorkList().clear();
	    cal.add(Calendar.DAY_OF_MONTH, -1);
	    getWorkList().addAll(patientDao.getWorklistDynamicallyByType(TimeUtil.setDayBeginning(cal).getTimeInMillis(), TimeUtil.setDayEnding(cal).getTimeInMillis(), searchOptions.getFilterIndex()));
	    log.info("Worklist yesterday - " + new Date(TimeUtil.setDayBeginning(cal).getTimeInMillis()) + " " + new Date(TimeUtil.setDayEnding(cal).getTimeInMillis()));
	    break;
	case SearchOptions.SEARCH_INDEX_CURRENTWEEK:
	    getWorkList().clear();
	    getWorkList().addAll(patientDao.getWorklistDynamicallyByType(TimeUtil.setWeekBeginning(cal).getTimeInMillis(), TimeUtil.setWeekEnding(cal).getTimeInMillis(), searchOptions.getFilterIndex()));
	    log.info("Worklist current week - " + new Date(TimeUtil.setWeekBeginning(cal).getTimeInMillis()) + " " + new Date(TimeUtil.setWeekEnding(cal).getTimeInMillis()));
	    break;
	case SearchOptions.SEARCH_INDEX_LASTWEEK:
	    getWorkList().clear();
	    cal.add(Calendar.WEEK_OF_YEAR, -1);
	    getWorkList().addAll(patientDao.getWorklistDynamicallyByType(TimeUtil.setWeekBeginning(cal).getTimeInMillis(), TimeUtil.setWeekEnding(cal).getTimeInMillis(), searchOptions.getFilterIndex()));
	    log.info("Worklist last week - " + new Date(TimeUtil.setWeekBeginning(cal).getTimeInMillis()) + " " + new Date(TimeUtil.setWeekEnding(cal).getTimeInMillis()));
	    break;
	case SearchOptions.SEARCH_INDEX_LASTMONTH:
	    getWorkList().clear();
	    cal.add(Calendar.MONDAY, -1);
	    getWorkList().addAll(patientDao.getWorklistDynamicallyByType(TimeUtil.setMonthBeginning(cal).getTimeInMillis(), TimeUtil.setMonthEnding(cal).getTimeInMillis(), searchOptions.getFilterIndex()));
	    log.info("Worklist last month - " + new Date(TimeUtil.setMonthBeginning(cal).getTimeInMillis()) + " " + new Date(TimeUtil.setMonthEnding(cal).getTimeInMillis()));
	    break;
	case SearchOptions.SEARCH_INDEX_DAY:
	    cal.setTime(searchOptions.getDay());
	    getWorkList().clear();
	    getWorkList().addAll(patientDao.getWorklistDynamicallyByType(TimeUtil.setDayBeginning(cal).getTimeInMillis(), TimeUtil.setDayEnding(cal).getTimeInMillis(), searchOptions.getFilterIndex()));
	    log.info("Day - " + new Date(TimeUtil.setDayBeginning(cal).getTimeInMillis()) + " " + new Date(TimeUtil.setDayEnding(cal).getTimeInMillis()));
	    break;
	case SearchOptions.SEARCH_INDEX_MONTH:
	    ...
	    getWorkList().addAll(patientDao.getWorklistDynamicallyByType(TimeUtil.setMonthBeginning(cal).getTimeInMillis(), TimeUtil.setMonthEnding(cal).getTimeInMillis(), searchOptions.getFilterIndex()));
	    log.info("Worklist month - " + new Date(TimeUtil.setMonthBeginning(cal).getTimeInMillis()) + " " + new Date(TimeUtil.setMonthEnding(cal).getTimeInMillis()));
	    break;
	case SearchOptions.SEARCH_INDEX_TIME:
	    ...
	    getWorkList().addAll(patientDao.getWorklistDynamicallyByType(fromTime, toTime, searchOptions.getFilterIndex()));
